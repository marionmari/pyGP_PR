...
from numpy import ones, zeros, array, reshape, dot, abs, floor
...
def meanMask(meanfunc, meanhyper=None, x=None, der=None):
    ...
    if meanhyper == None:           # report number of parameters
        A = src.Tools.general.feval(meanfunc[1])
        return [A]
    ...
    if der == None:                 # compute covariance matix for dataset x
        A = src.Tools.general.feval(mean, meanhyper, x[:,mask])
    else:                           # compute derivatives
        A = src.Tools.general.feval(mean, meanhyper, x[:,mask], der)
...
def meanProd(meanfunc, meanhyper=None, x=None, der=None):
    ...
    if meanhyper == None:    # report number of parameters
        A = [src.Tools.general.feval(meanfunc[0])]
        for ii in range(1,len(meanfunc)):
            A.append(src.Tools.general.feval(meanfunc[ii]))
        return A
    ...
    for ii in range(1,len(meanfunc)+1):
        no_param = src.Tools.general.feval(meanfunc[ii-1])
        DetermineNumberOfParameters(v,no_param)

    if der == None:                             # compute mean vector
        A = ones((n, 1))                     # allocate space for mean vector
        for ii in range(1,len(meanfunc)+1):     # iteration over multiplicand functions
            f = meanfunc[ii-1]
            B = src.Tools.general.feval(f,meanhyper[sum(v[0:ii]):sum(v[0:ii])+v[ii]], x)  # accumulate means
            A *= B

    elif isinstance(der, int):                  # compute derivative vector   
        tmp = 0
        A = ones((n, 1))                     # allocate space for derivative vector
        flag = True
        for ii in range(1,len(meanfunc)+1):
            tmp += v[ii]
            if der<tmp and flag:
                ...
                # compute derivative
                s = sum(v[0:ii])
                A *= src.Tools.general.feval(f, meanhyper[s:(s+v[ii])], x, int(jj))
            else:
                f = meanfunc[ii-1]              # i: which mean function
                s = sum(v[0:ii])
                A *= src.Tools.general.feval(f, meanhyper[s:(s+v[ii])], x)
    else:                            
        A = zeros((n,1))
    return A
...
def meanSum(meanfunc, meanhyper=None, x=None, der=None):
    ...
    if der == None:                             # compute mean vector
        A = zeros((n, 1))                    # allocate space for mean vector
        for ii in range(1,len(meanfunc)+1):     # iteration over summand functions
            f = meanfunc[ii-1]
            s = sum(v[0:ii])
            A = A + src.Tools.general.feval(f, meanhyper[s:(s+v[ii])], x)  # accumulate means
        ...
        # compute derivative
        s = sum(v[0:ii])
        A = A + src.Tools.general.feval(f, meanhyper[s:(s+v[ii])], x, int(jj))  # accumulate means
...
def meanScale(meanfunc, meanhyper=None, x=None, der=None):
    ...
    if meanhyper == None:                                   # report number of parameters
        A = [1]
        A.append( src.Tools.general.feval(meanfunc[0]) )
        return A
    ...
    if der == None:                                         # compute mean vector
        f = meanfunc[0]
        A = c * src.Tools.general.feval(f, meanhyper[1:], x)    # accumulate means

    elif isinstance(der, int) and der == 0:                 # compute derivative w.r.t. c
        f = meanfunc[0]
        A = src.Tools.general.feval(f, meanhyper[1:], x)

    else:                                   
        f = meanfunc[0]
        A = c * src.Tools.general.feval(f, meanhyper[1:], x, der-1)
    return A
...
def meanPow(meanfunc, meanhyper=None, x=None, der=None):
    ...
    d = abs(floor(meanhyper[0])) 
    d = max(d,1)

    if der == None:                                             # compute mean vector
        f = meanfunc[0]
        A = ( src.Tools.general.feval(f, meanhyper[1:], x) )**d     # accumulate means

    else:                                                       # compute derivative vector
        f = meanfunc[0]
        A = d * (src.Tools.general.feval(f, meanhyper[1:], x))**(d-1) \
                * src.Tools.general.feval(f, meanhyper[1:], x, der-1)
    return A
