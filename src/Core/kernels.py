...
def covSum(covfunc, hyp=None, x=None, z=None, der=None):
    ...
    if hyp == None: # report number of parameters
        A = [src.Tools.general.feval(covfunc[0])]
        for i in range(1,len(covfunc)):
            A.append(src.Tools.general.feval(covfunc[i]))
        return A
    ...
    for ii in range(1,len(covfunc)+1):
        no_param = src.Tools.general.feval(covfunc[ii-1])
        DetermineNumberOfParameters(v,no_param)

    AT = src.Tools.general.feval(covfunc[0], hyp[:v[1]], x, z)
    ...
    if der == None:                         # compute covariance matrix
        for ii in range(1,len(covfunc)+1):  # iteration over summand functions
            f = covfunc[ii-1]
            s = sum(v[0:ii])
            A = A + src.Tools.general.feval(f, hyp[s:(s+v[ii])], x, z) # accumulate covariances
        ...
        # compute derivative
        A = src.Tools.general.feval(f, hyp[sum(v[0:ii]):sum(v[0:ii])+v[ii]], x, z, int(j))
...
def covProd(covfunc, hyp=None, x=None, z=None, der=None):
    ...
    if hyp == None:    # report number of parameters
        ...
        for ii in range(1,len(covfunc)):
            A.append(src.Tools.general.feval(covfunc[ii]))
        return A
    ...
    if der == None:                             # compute covariance matrix
        for ii in range(1,len(covfunc)+1):      # iteration over multiplicand functions
            f = covfunc[ii-1]
            s = sum(v[0:ii])
            A *= src.Tools.general.feval(f, hyp[s:(s+v[ii])], x, z)  # accumulate covariances
    else:
        tmp = 0                                                                                  
        flag = True
        for ii in range(1,len(covfunc)+1): 
            ...
            if der<tmp and flag:
                ...
                # compute derivative
                s = sum(v[0:ii])
                A *= src.Tools.general.feval(f, hyp[s:(s+v[ii])], x, z, int(jj))
            else:                
                f = covfunc[ii-1]               # ii: which covariance function
                s = sum(v[0:ii])
                A *= src.Tools.general.feval(f, hyp[s:(s+v[ii])], x, z)            
    return A
...
def covScale(covfunc, hyp=None, x=None, z=None, der=None):
    ...
    if hyp == None:    # report number of parameters
        A = [1]
        A.append( src.Tools.general.feval(covfunc[0]) )
        return A
    ...
    if der == None:                                                 # compute covariance matrix
        A = sf2 * src.Tools.general.feval(covfunc[0], hyp[1:], x,z)     # accumulate covariances

    else:
        if der == 0:                                                # compute derivative w.r.t. sf2
            A = 2. * sf2 * src.Tools.general.feval(covfunc[0], hyp[1:], x, z)
        else:                                                       # compute derivative w.r.t. scaled covFunction
            A = sf2 * src.Tools.general.feval(covfunc[0], hyp[1:], x, z, der-1)
...
def covMask(covfunc, hyp=None, x=None, z=None, der=None):
    ...
    if hyp == None:     # report number of parameters
        A = [src.Tools.general.feval(covfunc[1])]
        return A
    ...
    if der == None:     # compute covariance matix for dataset x
        if z == None:
            A = src.Tools.general.feval(cov, hyp, x[:,mask])
        else:           # compute covariance between data sets x and z
            if z == 'diag':
                A = src.Tools.general.feval(cov,hyp,x[:,mask],z)
            else:       # coumpute cross covariances
                A = src.Tools.general.feval(cov,hyp,x[:,mask],z[:,mask])       
    else:               # compute derivatives
        if z == None:
            A = src.Tools.general.feval(cov, hyp, x[:,mask],z,der)
        elif z == 'diag':
            A = src.Tools.general.feval(cov,hyp,x[:,mask],z,der)
        else:
            A = src.Tools.general.feval(cov,hyp,x[:,mask],z[:,mask],der)
...
def covFITC(covfunc, xu=None, hyp=None, x=None, z=None, der=None):
    ...
    if hyp == None: # report number of parameters
        A = [src.Tools.general.feval(covfunc)]
        return A
    ...
    if der == None:                        # compute covariance matrices for dataset x
        if z == None:
            K   = src.Tools.general.feval(covfunc,hyp,x,'diag')
            Kuu = src.Tools.general.feval(covfunc,hyp,xu)
            Ku  = src.Tools.general.feval(covfunc,hyp,xu,x)
        elif z == 'diag':
            K = src.Tools.general.feval(covfunc,hyp,x,z)
            return K
        else:
            K = src.Tools.general.feval(covfunc,hyp,xu,z)
            return K
    else:                                  # compute derivative matrices
        if z == None:
            K   = src.Tools.general.feval(covfunc,hyp,x,'diag',der)
            Kuu = src.Tools.general.feval(covfunc,hyp,xu,None,der)
            Ku  = src.Tools.general.feval(covfunc,hyp,xu,x,der)
        elif z == 'diag':
            K = src.Tools.general.feval(covfunc,hyp,x,z,der)
            return K
        else:
            K = src.Tools.general.feval(covfunc,hyp,xu,z,der)
            return K
    return K, Kuu, Ku
